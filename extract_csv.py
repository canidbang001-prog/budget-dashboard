...
Usage:
  python extract_csv.py [xlsx_path] [csv_path]
  env: XLSX_PATH, CSV_PATH (둘 다 미지정 시 기본값 사용)
"""
import os
import re
import sys
...
# ── 경로: CLI > env > 기본값 ─────────────────────────────
DEFAULT_XLSX = os.path.join(os.path.dirname(__file__), '2026 전체합본예산서.xlsx')
DEFAULT_CSV = os.path.join(os.path.dirname(__file__), 'budget.csv')

XLSX_PATH = (
    sys.argv[1] if len(sys.argv) > 1
    else os.environ.get('XLSX_PATH', DEFAULT_XLSX)
)
CSV_PATH = (
    sys.argv[2] if len(sys.argv) > 2
    else os.environ.get('CSV_PATH', DEFAULT_CSV)
)
# ──────────────────────────────────────────────────────────
...
def extract_sheet(zf, ws_file, ss):
    ...
    for row in rows:
        ...
        if v.startswith('부서:'):
            ctx[0] = v.replace('부서:', '').strip()
        elif v.startswith('정책:'):
            ctx[1] = v.replace('정책:', '').strip()
        elif v.startswith('단위:'):
            ctx[2] = v.replace('단위:', '').strip()
    ...
    for row in rows:
        ...
        if any(PAGE_FOOTER_RE.match(f) for f in footers if f):
            continue
        # 완전 빈 행 스킵
...
def main():
    if not os.path.exists(XLSX_PATH):
        print(f"❌ XLSX 없음: {XLSX_PATH}")
        print(f"   사용법: python extract_csv.py [xlsx_path] [csv_path]")
        print(f"   또는: XLSX_PATH=... CSV_PATH=... python extract_csv.py")
        sys.exit(1)

    print(f"📋 XLSX → CSV v2 (인덴트 포함)")
